...
async function main() {
  console.log("Starting Full Authentic Commercial Photography Deployment for ALL Active Products...");
  ...
  console.log(`Processing ${products.length} products with ${allMedia.length} authentic media references.`);
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    const words = slug.split('-').filter(w => w.length > 2 && !['and', 'with', 'for', 'the', 'pack', 'set'].includes(w));
    
    // Exact or high relevance matches
  ...
  console.log(`\nSuccessfully upgraded ${updated}/${products.length} products with authentic commercial product photography.`);
}
